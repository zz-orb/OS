...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if ((0x180003f8u <= pa && pa + len <= 0x18000418u) ||
	    (0x180001f0u <= pa && pa + len <= 0x180001f8u) ) {
...
void sys_add_job(int env_id, char *cmd) {
    for (int i = 0; i < MAXJOBS; i++) {
        if (jobs[i].job_id == 0) {
    ...
    for (int i = 0; i < MAXJOBS; i++) {
        if (jobs[i].job_id != 0) {
            printk("[%d] %-10s 0x%08x %s\n", jobs[i].job_id, 
                   jobs[i].status == JOB_RUNNING ? "Running" : "Done", 
...
void sys_kill_job(int job_id) {
	...
	job->status = JOB_DONE;
    struct Env *e;
	try(envid2env(job->env_id, &e, 1));
	printk("[%08x] destroying %08x\n", curenv->env_id, e->env_id);
	env_destroy(e);
}

int sys_find_job_envid(int job_id) {
	job_t* job = NULL;
    for (int i = 0; i < MAXJOBS; i++) {
        if (jobs[i].job_id == job_id) {
			job = &jobs[i];
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_list_jobs] = sys_list_jobs,
	[SYS_kill_job] = sys_kill_job,
	[SYS_find_job_envid] = sys_find_job_envid,
};
